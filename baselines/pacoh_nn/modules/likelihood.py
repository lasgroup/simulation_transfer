...
class GaussianLikelihood(tf.Module):
    ...
    def __init__(self, output_dim, n_batched_models, std=0.2, trainable=True, name='gaussian_likelihood'):
        ...
        _ones = tf.ones((n_batched_models, output_dim), dtype=tf.float32)
        if trainable:
            self.log_std = tf.Variable(_ones * tf.math.log(std), trainable=True)
        else:
            self.log_std = tf.constant(_ones * tf.math.log(std))
